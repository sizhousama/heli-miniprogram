...
nav:function(ev){ //跳转至页面；引用方法 bindtap="nav" data-to='../mine/mine'
   wx.navigateTo({url: ev.currentTarget.dataset.to})
...
editbh(e){
  var sco = this;
  var cur = e.currentTarget.dataset.params;
  var a = ''
  // 原材
  if (cur == 'YS_obja.params.ccph') { a = sco.data.YS_obja.params.ccph };
  if (cur == 'YS_obja.params.ypbh') { a = sco.data.YS_obja.params.ypbh };
  if (cur == 'YS_obja.params.jieguo') { a = sco.data.YS_obja.params.jieguo };
  if (cur == 'YS_obja.params.beizhu') { a = sco.data.YS_obja.params.beizhu };
  // 标养
  if (cur == 'YS_obja.params.xinpian') { a = sco.data.YS_obja.params.xinpian };
  if (cur == 'YS_obja.params.bybh') { a = sco.data.YS_obja.params.bybh };
  if (cur == 'YS_obja.params.byjg') { a = sco.data.YS_obja.params.byjg };
  // 同养
  if (cur == 'YS_obja.params.tyxinpian') { a = sco.data.YS_obja.params.tyxinpian };
  if (cur == 'YS_obja.params.tybh') { a = sco.data.YS_obja.params.tybh };
  if (cur == 'YS_obja.params.tyjg') { a = sco.data.YS_obja.params.tyjg };
  // 抗渗
  if (cur == 'YS_obja.params.ksbh') { a = sco.data.YS_obja.params.ksbh };
  if (cur == 'YS_obja.params.ksjg') { a = sco.data.YS_obja.params.ksjg };
  // 柱头
  if (cur == 'YS_obja.params.ztxinpian') { a = sco.data.YS_obja.params.ztxinpian };
  if (cur == 'YS_obja.params.ztbh') { a = sco.data.YS_obja.params.ztbh };
  if (cur == 'YS_obja.params.ztjg') { a = sco.data.YS_obja.params.ztjg };
  // 拆模
  if (cur == 'YS_obja.params.cmbh') { a = sco.data.YS_obja.params.cmbh };
  if (cur == 'YS_obja.params.cmjg') { a = sco.data.YS_obja.params.cmjg };
  // 焊接
  if (cur == 'YS_obja.params.dzpaihao') { a = sco.data.YS_obja.params.dzpaihao };
  if (cur == 'YS_obja.params.dzguige') { a = sco.data.YS_obja.params.dzguige };
  if (cur == 'YS_obja.params.dzchangjia') { a = sco.data.YS_obja.params.dzchangjia };
  if (cur == 'YS_obja.params.dzbh') { a = sco.data.YS_obja.params.dzbh };
  if (cur == 'YS_obja.params.dzjg') { a = sco.data.YS_obja.params.dzjg };
  // 砌筑
  if (cur == 'YS_obja.params.qzbh') { a = sco.data.YS_obja.params.qzbh };
  if (cur == 'YS_obja.params.qzjg') { a = sco.data.YS_obja.params.qzjg };
  // 外墙
  if (cur == 'YS_obja.params.wqbh') { a = sco.data.YS_obja.params.wqbh };
  if (cur == 'YS_obja.params.wqjg') { a = sco.data.YS_obja.params.wqjg };
  // 内墙
  if (cur == 'YS_obja.params.nqbh') { a = sco.data.YS_obja.params.nqbh };
  if (cur == 'YS_obja.params.nqjg') { a = sco.data.YS_obja.params.nqjg };
  // 找平
  if (cur == 'YS_obja.params.zpbh') { a = sco.data.YS_obja.params.zpbh };
  if (cur == 'YS_obja.params.zpjg') { a = sco.data.YS_obja.params.zpjg };

  sco.setData({
    tapshow:1,
    bhtitle:e.currentTarget.dataset.bhtitle,
    bhparams: e.currentTarget.dataset.params,
    bhvalue:a
  })
},
...
// 池家昊事件代码
  ...
  setTapshow2: function (e) {
    var sco = this;
    var tap = e.currentTarget.dataset.tap;
    var codeid = e.currentTarget.dataset.sdata.codeid;
   // sco.data.YS_objb.params.jihuariqi = e.currentTarget.dataset.sdata.jihuariqi;
    sco.data.YS_objb.params.jiaozhuriqi = e.currentTarget.dataset.sdata.jiaozhuriqi;

    sco.data.YS_objb.params.codeid = codeid;
    if (e.currentTarget.dataset.tap == this.data.tapshow) { tap = 0 };
    sco.setData({
      tapshow: tap,
      'YS_objb.params.jihuariqi': e.currentTarget.dataset.sdata.jihuariqi,
      'YS_objb.params.jiaozhuriqi': e.currentTarget.dataset.sdata.jiaozhuriqi
...
searchSubmit: function (e) {
  ...
  var sco = this;
  sco.data.YS_plista.params = {keyword:e.detail.value,curPage:1,listnum:5};

  sco.data.joblei == '最近'?sco.data.YS_plista.params.city_id = sco.data.YS_obja.data.cityid:"";
...
cancelSearch: function (e) {
  ...
  var sco = this;
  sco.data.YS_plista.params = [];
  if(sco.data.favorite){
    sco.data.YS_plista.params.worktype = sco.data.favorite[0].worktype;
    sco.data.YS_plista.params.noticetype = sco.data.favorite[0].noticetype;
    sco.data.YS_plista.params.province_id = sco.data.favorite[0].province_id;
    sco.data.YS_plista.params.city_id = sco.data.favorite[0].city_id;
  }else{
    sco.data.YS_plista.params = {curPage:1,listnum:5};
  }
...
//委托单、检测报告上传
baogaoUpload: function (ev) {    //bindtap="weituoUpload" data-key="picturearr" data-arr="1" data-siyou="1"
    var sco = this;
var key = ev.currentTarget.dataset.key;
var siyou = ev.currentTarget.dataset.siyou;
var arrstr = ev.currentTarget.dataset.arr; 
var qntoken = YSfac.getstore('qntoken');
var field = sco.data.littab;
...
if(re.key){
  ...
  if(key=='weituo'){
     var weituo = YSfac.getstore('weituoarr');
     var weituourl = YSfac.getstore('weituourl');
     if(!weituo)weituo = [];
     if(!weituourl)weituourl = [];
     weituo.push(filePath);
     weituourl.push(re.imageURL);
     YSfac.setstore('weituoarr',weituo);
     YSfac.setstore('weituourl',weituourl);    
   }else{
     var jiance = YSfac.getstore('jiancearr');
     var jianceurl = YSfac.getstore('jianceurl');
     if(!jiance)jiance = [];
     if(!jianceurl)jianceurl = [];
     jiance.push(filePath);
     jianceurl.push(re.imageURL);
     YSfac.setstore('jiancearr',jiance);
     YSfac.setstore('jianceurl',jianceurl);       
   }



   sco.data.imageList.push(filePath);
   sco.setData({imageList:sco.data.imageList});
...
baogaodelimg:function(e){ //添加模块的，删除图片
    this.data.imageList.splice(e.currentTarget.dataset.id,1);

    if(e.currentTarget.dataset.type=='weituo'){
     var weituo = YSfac.getstore('weituoarr');
     var weituourl = YSfac.getstore('weituourl');
      weituo.splice(e.currentTarget.dataset.id,1);
      weituourl.splice(e.currentTarget.dataset.id,1);
     ...
     var jiance = YSfac.getstore('jiancearr');
     var jianceurl = YSfac.getstore('jianceurl');
      jiance.splice(e.currentTarget.dataset.id,1);
      jianceurl.splice(e.currentTarget.dataset.id,1);
